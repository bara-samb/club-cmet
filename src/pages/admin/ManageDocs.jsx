...
const VIDE_B = { nom: '', categorie: 'cours', filiere: 'Commun', niveau: 'Commun', date: '', description: '' };
...
export default function ManageDocs() {
    ...
    const PdfField = () => (
        <div className="md:col-span-2">
            ...
            <label className="flex items-center gap-4 cursor-pointer border-2 border-dashed border-[#C8C8C8] hover:border-[#187840] bg-[#F8F0F0] hover:bg-[#187840]/5 rounded-2xl px-5 py-4 transition-all">
    ...
    const ActionBtns = ({ label, onCancel }) => (
        <div className="md:col-span-2 flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" disabled={uploading}
                className="bg-[#003058] text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-[#002850] transition-colors disabled:opacity-60 shadow-sm shadow-[#003058]/20 flex items-center gap-2">
    ...
    const DocRow = ({ item, col, onEdit }) => (
        <div className="flex items-center gap-4 px-5 py-4 hover:bg-[#F8F0F0]/80 transition-colors group">
            ...
            <div className="flex-grow min-w-0">
                <p className="text-sm font-bold text-[#003058] truncate">{item.nom}</p>
            ...
            {item.url && (
                <a href={item.url} target="_blank" rel="noopener noreferrer"
                    className="text-[11px] text-[#187840] font-bold hover:underline shrink-0 flex items-center gap-1"><ExternalLink size={12} /> Voir</a>
            )}
            <div className="flex gap-1.5 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                <button onClick={onEdit}
                    className="w-8 h-8 bg-white border border-[#C8C8C8] hover:border-[#187840] hover:text-[#187840] text-slate-400 rounded-lg flex items-center justify-center transition-colors shadow-sm">
                    <Pencil size={14} strokeWidth={2.5} />
                </button>
                <button onClick={() => setConfirmDel({ ...item, _col: col })}
                    className="w-8 h-8 bg-white border border-[#C8C8C8] hover:border-red-500 hover:text-red-500 text-slate-400 rounded-lg flex items-center justify-center transition-colors shadow-sm">
        ...
        <div className="min-h-screen bg-[#F8F0F0] p-6 animate-in fade-in duration-500">
            ...
            {confirmDel && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
                        ...
                        <h3 className="font-black text-xl text-[#003058] mb-2">Supprimer ce fichier ?</h3>
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex gap-2 flex-wrap">
                    {ONGLETS.map(o => (
                        <button key={o.key} onClick={() => { setOnglet(o.key); setEditDocId(null); setEditMaqId(null); setEditBibId(null); setPdfFile(null); }}
                            className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${onglet === o.key ? 'bg-[#003058] text-white shadow-sm shadow-[#003058]/20' : 'bg-white border border-slate-100 text-slate-600 hover:bg-[#F8F0F0] hover:border-[#C8C8C8]'}`}>
                            <o.icon size={16} />
                            {o.label}
                        </button>
                    ))}
                        ...
                        <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 md:p-8">
                            <h2 className="font-bold text-lg text-[#003058] mb-6 flex items-center gap-3">
                                <span className="w-10 h-10 bg-[#187840]/10 rounded-2xl flex items-center justify-center">
                                    {editDocId ? <Pencil className="w-5 h-5 text-[#187840]" /> : <Plus className="w-5 h-5 text-[#187840]" />}
                                </span>
                                ...
                                {CATEGORIES_RESSOURCES.map(cat => {
                                    ...
                                    return (
                                        <div key={cat.id} className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                                            <div className="px-6 py-4 bg-[#F8F0F0]/80 border-b border-slate-100 flex items-center gap-3">
                                                <FolderOpen className="w-5 h-5 text-amber-500" />
                                                <span className="font-bold text-sm text-[#003058]">{cat.label}</span>
                                                <span className="ml-auto text-[11px] text-[#187840] bg-[#187840]/10 px-2.5 py-0.5 rounded-full font-bold">{items.length}</span>
                                            </div>
                                            {items.length === 0
                ...
                {onglet === 'maquettes' && (
                    <>
                        <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 md:p-8">
                            <h2 className="font-bold text-lg text-[#003058] mb-6 flex items-center gap-3">
                                <span className="w-10 h-10 bg-[#187840]/10 rounded-2xl flex items-center justify-center">
                                    {editMaqId ? <Pencil className="w-5 h-5 text-[#187840]" /> : <Plus className="w-5 h-5 text-[#187840]" />}
                                </span>
                            ...
                            {['IT', 'HEC'].map(filiere => {
                                ...
                                return (
                                    <div key={filiere} className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                                        <div className={`px-6 py-4 border-b flex items-center gap-3 ${filiere === 'IT' ? 'bg-[#003058]' : 'bg-amber-600'}`}>
                ...
                {onglet === 'bibliotheque' && (
                    <>
                        <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-6 md:p-8">
                            <h2 className="font-bold text-lg text-[#003058] mb-2 flex items-center gap-3">
                                <span className="w-10 h-10 bg-[#187840]/10 rounded-2xl flex items-center justify-center">
                                    {editBibId ? <Pencil className="w-5 h-5 text-[#187840]" /> : <Plus className="w-5 h-5 text-[#187840]" />}
                                </span>
                            ...
                            <form onSubmit={handleSubmitBiblio} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <select value={formBiblio.niveau} onChange={e => setFormBiblio({ ...formBiblio, niveau: e.target.value })}
                                        className="input-field">
                        ...
                        <div className="space-y-6">
                            {CATEGORIES_BIBLIO.map(cat => {
                                ...
                                return (
                                    <div key={cat.id} className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                                        <div className="px-6 py-4 bg-[#F8F0F0]/80 border-b border-slate-100 flex items-center gap-3">
                                            <BookOpen className="w-5 h-5 text-[#003058]" />
                                            <span className="font-bold text-sm text-[#003058]">{cat.label}</span>
                                            <span className="ml-auto text-[11px] text-[#187840] bg-[#187840]/10 px-2.5 py-0.5 rounded-full font-bold">{items.length}</span>
                                        </div>
                                        <div className="divide-y divide-slate-50">
                                            {items.map(b => (
                                                <DocRow key={b.id} item={b} col="bibliotheque"
                                                    onEdit={() => { setEditBibId(b.id); setFormBiblio({ nom: b.nom, categorie: b.categorie, filiere: b.filiere || 'Commun', niveau: b.niveau || 'Commun', date: b.date || '', description: b.description || '' }); setPdfFile(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />
                                            ))}
                                        </div>
                                    </div>
                                );
